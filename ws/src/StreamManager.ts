...
import { getSpotifyId, getVideoId, isValidSpotifyURL, isValidYoutubeURL } from "./utils";
import { checkUrlPlatform } from "./utils";
...
export class RoomManager {
  ...
  async processJob(job: Job) {
    ...
    if (name === "cast-vote") {
      ...
      console.log("adminAddStreamHandler is called first time")
    ...
    if (!isValidYoutubeURL(url) && !isValidSpotifyURL(url)) {
      currentUser?.ws.forEach((ws) => {
        ws.send(
          JSON.stringify({
            type: "error",
            data: { message: "Invalid YouTube URLss" },
          })
        );
      });
      return;
